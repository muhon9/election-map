...
export const PATCH = withPermApi(async (req, { params }, session) => {
  ...
  const before = await User.findById(params.id)
  ...
  const beforeLevel =
    typeof before.role?.level === "number" ? before.role.level : 100;
  if (beforeLevel < actorLevel) {
    return new Response(
      JSON.stringify({ error: "Insufficient level to modify this user" }),
      { status: 403 }
    );
  ...
  let roleChangeFrom = before.role?.name || "";
  let roleChangeTo = "";

  // Role change
  if (body.roleId) {
    if (actorId === before._id.toString()) {
      return new Response(
        JSON.stringify({ error: "You cannot change your own role" }),
        { status: 403 }
      );
    ...
    updates.role = newRole._id;
    roleChanged = String(before.role?._id || "") !== String(newRole._id);
    roleChangeTo = newRole.name || "";
  }

  const after = await User.findByIdAndUpdate(params.id, updates, { new: true })
    .populate("role", "name level")
    .lean();
  ...
  await logAudit(req, session, {
    ...
    before: {
      ...
      email: before.email,
      phone: before.phone,
      role: before.role?.name || null,
    },
    after: {
      username: after.username,
      email: after.email,
      phone: after.phone,
      role: after.role?.name || null,
    },
  });

  // --- audit: role change (separate, clearer in UI)
  if (roleChanged) {
    await logAudit(req, session, {
      action: "role_change",
      entityType: "user",
      entityId: after._id,
      summary: `Changed role ${roleChangeFrom || "none"} → ${
        roleChangeTo || "none"
      } for ${after.username}`,
      before: { role: roleChangeFrom || null },
      after: { role: roleChangeTo || null },
    });
  }

  after.password = undefined;
