...
export default function AreasPage() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        const url = `/api/areas?page=${page}&limit=${limit}&q=${encodeURIComponent(
          q
        )}&sort=${sort}&dir=${dir}`;
        ...
        if (res.ok) {
          // normalize pages in case API doesn't send it
          const pages =
            json.pages ||
            (json.total && json.limit
              ? Math.max(1, Math.ceil(json.total / json.limit))
              : 1);
          setData({
            items: json.items || [],
            total: json.total || 0,
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          <input
            ...
            placeholder="Search (name, code, center)…"
            value={q}
            onChange={(e) => {
              setQ(e.target.value);
              setPage(1);
        ...
        <table className="min-w-[900px] w-full text-sm">
          <thead className="bg-gray-100">
            <tr>
              ...
              <Th sort={sort} dir={dir} field="code" onSort={toggleSort}>
                Code
              </Th>
...
function Row({ a, canEdit, canDelete }) {
  ...
  const centerName =
    a.centerName || (a.center && (a.center.name || a.center)) || "—";
