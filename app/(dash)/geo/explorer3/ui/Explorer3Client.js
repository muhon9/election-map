...
export default function Explorer3Client() {
  ...
  const [upazillaId, setUpazillaId] = useState(sp.get("upazillaId") || "");
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setUpazillas(items);
        if (upazillaId && !items.find((x) => x._id === upazillaId)) {
          setUpazillaId("");
          setUnionId("");
        }
      } catch (e) {
  ...
  // Load Unions by Upazilla (level 2)
  useEffect(() => {
    (async () => {
      if (!upazillaId) {
        setUnions([]);
      ...
      try {
        ...
        const res = await fetch(
          `/api/geo?parentId=${encodeURIComponent(upazillaId)}&active=1`,
          { cache: "no-store" }
        );
        const j = await res.json();
  ...
  async function createUnion(e) {
    e.preventDefault();
    if (!upazillaId || !newUnionName.trim()) return;
    setSavingUnion(true);
    try {
    ...
    <div className="p-4 space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-semibold">
          Three-Step Hierarchy (Upazilla → Union → Ward)
        </h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <Pane
          title={
            upazillaId ? "Unions of selected Upazilla" : "Pick an Upazilla"
          }
          loading={loadingN}
          items={unions}
          selectedId={unionId}
