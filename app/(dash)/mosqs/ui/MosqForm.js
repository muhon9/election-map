...
export default function MosqForm({ mode = "create", id }) {
    ...
    (async () => {
      try {
        ...
        if (isEdit) {
          ...
          const jUpa = j.upazillaId?._id || j.upazillaId || "";
          ...
          if (jCity) {
            ...
            // clear upazilla/union
            setUpazillaId("");
            setUnionId("");
          } else if (jUpa) {
            setGeoMode("upazilla");
            setUpazillaId(String(jUpa));
            // unions under upazilla
            ...
            if (jUni && us.find((u) => u._id === String(jUni))) {
              ...
              // if no union stored but ward is directly under upazilla (allowed by your validator),
              // load wards from upazilla:
              if (jWar) {
  ...
  // On upazilla change → load unions (and reset ward)
  useEffect(() => {
    let alive = true;
    (async () => {
      if (!upazillaId || geoMode !== "upazilla") {
        setUnions([]);
        setUnionId("");
        setWards([]);
        setWardId("");
  ...
  async function onSubmit(e) {
    ...
    const geoPayload =
      geoMode === "city"
        ...
        : {
            upazillaId: upazillaId || null,
            unionId: unionId || null,
            wardId: wardId || null, // can be under union or (rarely) upazilla depending on your data
            cityId: null,
          };
            ...
            <>
              ...
              <div>
                ...
                <select
                  ...
                  disabled={!upazillaId}
                  title={!upazillaId ? "Select an upazilla first" : ""}
                >
                  <option value="">
                    {upazillaId ? "— None —" : "Select an upazilla first"}
                  </option>
                  {unions.map((u) => (
                    <option key={u._id} value={u._id}>
                      {u.name}
              ...
              <div className="md:col-span-2">
                ...
                <select
                  ...
                  value={wardId}
                  onChange={(e) => setWardId(e.target.value)}
                  disabled={!upazillaId /* and optionally !unionId */}
                  title={
                    !upazillaId
                ...
                <p className="text-xs text-gray-500 mt-1">
                  ...
                  model places wards directly under an Upazilla, select the
                  Upazilla and leave Union empty—then pick from the loaded
                  wards.
                </p>
              </div>
            </>
